...
| `CODEX-PR017-02` | *(partial — see below)* §2.1's "only the V1 use case in §6 is being built" replaced; **new §6.6** tabulates both framings with the status of each; `BL-11` note rewritten to state it is neither retired nor completed and remains unauthorized | `PREREQ-001` §2.1, **§6.6**; `BACKLOG.md`; `README.md` |
...
### `CODEX-PR017-02` is only partly corrected

The finding's required correction has **two** parts. Recording which one is
outstanding is the point of this section.

1. *"replace 'the V1 use case in §6 is being built'"* — **done.** §2.1's closing
   sentence now points at §6.6, which states that supplier diligence is frozen
   direction, not implemented, and not authorized under `BL-11`.
2. *"Label the software-engineering V1 material as the historical/current
   TASK-001 framing"* — **not done.** §2.1 still opens *"In V1 — autonomous
   software engineering:"* with no marker that this is the TASK-001 framing
   rather than the current demonstration. §6.6's table and §6.4 explain the
   relationship, and §2.1 links to them, but the heading itself is unlabelled.

The corrective commit was scoped to the three findings and did not reach this.
The subsequent authorization that completed this record was explicitly limited
to the review record and forbade modifying the product definition, so it could
not be fixed there either. **It remains open** and needs its own authorization.
